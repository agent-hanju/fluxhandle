 ...
 * EmitHandle allows external code to directly emit items, errors, and
 * completion signals through public {@link #emitNext(Object)},
 * {@link #emitError(Throwable)}, and {@link #emitComplete()} methods.
 *
 * <p>
 ...
 * EmitHandle<ChatCompletionChunk> handle = new EmitHandle<>(ChatCompletionChunk.class, listener);
 *
 ...
 * @see FluxListener
 */
public class EmitHandle<T> implements IFluxHandle<T, T> {
  private static final Logger log = LoggerFactory.getLogger(EmitHandle.class);
  ...
  /**
   * Creates a new EmitHandle with the given type and listener.
   *
  ...
  public synchronized void emitNext(final T item) {
    ...
    try {
      ...
      this.emitError(new FluxHandleException("delta merge failed", e));
      return;
    }
    ...
    try {
      ...
      this.emitError(new FluxListenerException("listener failed while emit next", ex));
      return;
    }
  ...
  public synchronized void emitComplete() {
    if (this.completed) {
      ...
      try {
        ...
        this.emitError(new FluxHandleException("merge build failed while complete", e));
        return;
      }
      try {
        this.listener.onComplete();
      } catch (final Exception e) {
        this.emitError(new FluxListenerException("listener failed while complete", e));
        return;
      }
  ...
  public synchronized void cancel() {
    if (this.completed) {
      ...
      try {
        ...
        this.emitError(new FluxHandleException("build failed while cancel", e));
        return;
      }
      try {
        this.listener.onCancel();
      } catch (final Exception ex) {
        this.emitError(new FluxListenerException("listener failed while cancel", ex));
        return;
      }
      this.cancelled = true;
      this.completed = true;
